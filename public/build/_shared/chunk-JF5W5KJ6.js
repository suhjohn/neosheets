// @/constants.ts
var DEFAULT_COLUMN_COUNT = 26;
var DEFAULT_ROW_COUNT = 1e3;
var DEFAULT_FONT_FAMILY = "Inter";
var DEFAULT_FONT_SIZE = 12;
var DEFAULT_LINE_HEIGHT = 1.5;
var DEFAULT_CELL_HEIGHT = 21;
var DEFAULT_COLUMN_WIDTH = 120;
var DEFAULT_CELL_PADDING = 2;
var DEFAULT_CELL_BORDER_WIDTH = 1;
var COLUMN_HEADER_HEIGHT = 21;
...
function generateExcelHeaders(length) {
  const headers = [];
...
var extractMustacheVariables = (text) => {
  const regex = /\{\{(.*?)\}\}/g;
  ...
  try {
    ...
    if (e instanceof SyntaxError) {
      ...
      if (errorMessage.includes("Unexpected token")) {
        return {
          isValid: false,
          error: "Invalid character found. Check for misplaced commas or quotes."
        };
      } else if (errorMessage.includes("Unexpected end of JSON input")) {
        return {
          isValid: false,
          error: "JSON is incomplete. Make sure all brackets and braces are closed."
        };
      } else if (errorMessage.includes("Unexpected string in JSON")) {
        return {
          isValid: false,
          error: "Unexpected text found. Make sure all the keys in the JSON are in quotes."
        };
      }
    }
    return {
...
var generateFillSequence = (dataToFill, fillRange) => {
  const filledData = {};
  const sourceData = [];
  const sourceKeys = [];
  ...
  const fillLength = fillRange.end.row - fillRange.start.row;
  console.log({
    patternLength,
    fillLength,
    sourceData
  ...
  for (let i = 0; i < fillLength; i++) {
    ...
    const targetRow = fillRange.start.row + i + 1;
    ...
    const value = originalCell.formula ? "rerun the cell" : extendSequence(sourceData, i + patternLength, targetRow - sourceRow);
...
var extendSequence = (sequence, index, step) => {
...
var extendSingleCellPattern = (value, step) => {
...
var isDateSequence = (sequence) => {
  return sequence.every((val) => isValidDate(val));
};
var extendDateSequence = (sequence, index) => {
...
var isValidFormula = (value) => {
  return value.startsWith("=");
};
var adjustFormula = (formula, step) => {
  return formula.replace(/([A-Za-z]+)(\d+)/g, (match, col, row) => {
    const newRow = parseInt(row, 10) + step;
    return `${col}${newRow}`;
  });
};
var isAlphabetSequence = (sequence) => {
  return sequence.every((val) => /^[A-Za-z]+$/.test(val));
};
var extendAlphabetSequence = (sequence, index) => {
...
function getCellsForRow(cellStates, rowIndex) {
  const rowCells = {};
...
//# sourceMappingURL=/build/_shared/chunk-JF5W5KJ6.js.map
